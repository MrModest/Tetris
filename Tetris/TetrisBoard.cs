...
namespace Tetris
{
    public class Player // For ScoreTable
    {
    ...
    public class TetrisBoard
    {
        ...
        public static int Height { get; set; } = 20; //blocks
        public static int Width { get; set; } = 10; //blocks
        ...
        public void InitGrid()
        {
            for (int i = 0; i < Height; i++)
            {
                for (int j = 0; j < Width; j++)
                {
                    Grid[i, j] = false; // empty board (not contain any blocks and figures)
                }
            }

            foreach (var block in Lying)
            {
                Grid[block.vPos, block.hPos] = true; //set lying blocks
            }
        }

        public void GetNextFigure()
        {
            if (NextFigure != null) // if NextFigure = null, then CurrentFigure = null too.
            {
            ...
            CurrentFigure.PositionChanged += new EventHandler(Draw); // if position changed then redraw figure
        ...
        public void DrawNext() // draw on PBox for NextFigure
        {
